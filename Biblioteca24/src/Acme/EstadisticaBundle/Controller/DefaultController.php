...
class DefaultController extends Controller
{
	...
	public function sexoAction()
	{
		...
		$date_to = new \DateTime('2015-12-31');
		$query = $qb->select('s')
		...
		$ob->title->text('Cantidad de Préstamos según sexo en Diciembre 2015');
		...
		return $this->render('EstadisticaBundle:Default:index.html.twig', array(
	        'chart' => $ob
	    ));
	}
	...
	public function edadAction()
	{
		...
		$ob->title->text('Cantidad de Préstamos por Rango Etario en el Mes de Diciembre');
		$ob->subtitle->text('Total: '.sizeof($prestamos).' préstamos');
	...
	public function fechaAction()
	{
		...
		$ob->title->text('Cantidad de Préstamos por Día en el Mes de Diciembre');
		$ob->subtitle->text('Total: '.sizeof($prestamos).' préstamos');
		$ob->xAxis->categories($categories);
		$ob->legend->enabled(true);
